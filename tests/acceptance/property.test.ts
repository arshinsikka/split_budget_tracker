    ...
    it('should maintain invariants through random expense and settlement sequences', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(sequenceArb, async (actions) => {
          ...
          expect(Math.abs(userA.walletBalance - oracleData.users[0].walletBalance)).toBeLessThan(0.01);
          expect(Math.abs(userB.walletBalance - oracleData.users[1].walletBalance)).toBeLessThan(0.01);
