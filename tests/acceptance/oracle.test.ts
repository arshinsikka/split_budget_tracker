...
describe('Oracle-based Tests', () => {
  ...
  describe('Oracle vs API comparison', () => {
    ...
    it('should handle odd splits like 101.00 correctly', async () => {
      ...
      expect(userA.budgetByCategory.food).toBe(50.50);
      expect(userB.budgetByCategory.food).toBe(50.50);
      
      // Compare with oracle
      expect(Math.abs(userA.budgetByCategory.food - oracleData.users[0].budgetByCategory.food)).toBeLessThan(0.01);
      expect(Math.abs(userB.budgetByCategory.food - oracleData.users[1].budgetByCategory.food)).toBeLessThan(0.01);
    });
    ...
    it('should maintain budget invariance during settlements', async () => {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        for (const category of beforeCategories) {
          expect(afterData.users[i].budgetByCategory[category]).toBe(beforeData.users[i].budgetByCategory[category]);
        }
      }
